...
floatcentered(const Arg *arg)
{
	...
	if (c->isfloating) {
		...
		// Центровка на мониторе
		int x = c->mon->mx + (c->mon->mw - w) / 2;
...
resizekey(const Arg *arg)
{
	...
	if (!(c = selmon->sel))
		return;
	if (c->isfullscreen) // нельзя изменять фуллскрин
		return;
	...
	// Получаем keycode клавиши R
	KeyCode keycode = XKeysymToKeycode(dpy, XK_r);
	...
	for (;;) {
		...
		// Если клавиша R отпущена — заканчиваем
		XQueryKeymap(dpy, keys_return);
		if (!(keys_return[keycode >> 3] & (1 << (keycode & 7))))
			break;
...
movekey(const Arg *arg)
{
	...
	// Узнаём код клавиши D
	KeyCode keycode = XKeysymToKeycode(dpy, XK_d);

	for (;;) {
		XMaskEvent(dpy, MOUSEMASK | ExposureMask | SubstructureRedirectMask, &ev);

		// Проверка: если клавиша D отпущена — выходим
		XQueryKeymap(dpy, keys_return);
		if (!(keys_return[keycode >> 3] & (1 << (keycode & 7))))
			break;
